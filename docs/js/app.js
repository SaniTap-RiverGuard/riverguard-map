...
function assumptionsHtml() {
  ...
  const layers = [
    ...
    ['Land use & paddy', 'WorldCover composition of each buffer. Likely-paddy: cropland in near-flat (<2°) buffers that touch water/wetland — toggleable area deduction, heuristic only. FIELD VALIDATION (Aug 2026): TERRACED paddy on slopes is NOT reliably detected from desk data — the slope criterion systematically misses it, so cropland in hill country may be rice even when unflagged. Treat cropland areas as unverified until field-checked.', 'ESA WorldCover derived + SaniTap field check'],
